...
class Particle {
  ...
  generateRandomPos(x, y, mag) {
    ...
    const randomY = Math.random() * (typeof window !== "undefined" ? window.innerHeight : 500)
...
export function ParticleTextEffect({ words = DEFAULT_WORDS, loop = false, onComplete, duration = 280 }) {
  ...
  const nextWord = (word, canvas) => {
    ...
    // Draw text with responsive font size to prevent cutoff
    offscreenCtx.fillStyle = "white"
    const fontSize = Math.min(canvas.width / (word.length * 0.65), 150)
    offscreenCtx.font = `bold ${fontSize}px Inter`
    ...
    for (const coordIndex of coordsIndexes) {
      ...
      if (alpha > 0) {
        ...
        if (particleIndex < particles.length) {
          ...
          particle.maxSpeed = Math.random() * 10 + 6 // Faster convergence (was 6 + 4)
          particle.maxForce = particle.maxSpeed * 0.06 // Sharper turning (was 0.05)
  ...
  const animate = () => {
    ...
    if (isWarpingRef.current) {
      ...
      for (const particle of particles) {
        ...
        ctx.beginPath();
        // Electric Blue hypersonic streaks
        ctx.strokeStyle = `rgba(0, 112, 243, 0.8)`;
        ctx.lineWidth = 3;
        ctx.moveTo(particle.pos.x - particle.vel.x * 2, particle.pos.y - particle.vel.y * 2);
    ...
    if (frameCountRef.current % duration === 0 && !isWarpingRef.current) {
      if (wordIndexRef.current < words.length - 1) {
        ...
        if (!hasCompletedRef.current) {
          ...
          // Wait for warp to play out before transitioning
          setTimeout(() => {
  ...
  useEffect(() => {
    ...
    // Handle resize to keep it fullscreen
    const handleResize = () => {
